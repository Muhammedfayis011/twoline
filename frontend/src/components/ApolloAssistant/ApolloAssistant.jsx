...
import './ApolloAssistant.css';
import { useLanguage } from '../../context/LanguageContext';

export default function ApolloAssistant() {
  const { isAr } = useLanguage();
  const [isOpen, setIsOpen] = useState(false);
  const [messages, setMessages] = useState([
    { role: 'assistant', text: isAr ? 'مرحباً! أنا مستشار الذكاء الاصطناعي أبولو الخاص بك. كيف يمكنني مساعدتك في اختيار الأثاث المثالي اليوم؟' : 'Hello! I am Apollo, your AI Design Assistant. How can I help you pick the perfect furniture today?' }
  ]);
  ...
  return (
    <div className={`apollo-assistant-wrapper ${isAr ? 'rtl' : 'ltr'}`}>
      ...
      <button 
        ...
        onClick={toggleChat}
        aria-label="Toggle Apollo AI Assistant"
      >
        <span className="apollo-icon">✨</span>
        <span className="apollo-btn-text">{isAr ? 'أبولو الذكاء الاصطناعي' : 'Apollo AI'}</span>
      </button>
        ...
        <div className="apollo-chat-window">
          <div className="apollo-chat-header">
            <div className="apollo-header-title">
              <span className="apollo-icon">✨</span>
              <div>
                <strong>{isAr ? 'مساعد أبولو' : 'Apollo Assistant'}</strong>
